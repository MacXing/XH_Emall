...
<html>
  <head>
	...
	<style type="text/css">
		.pages1{
		overflow:hidden; color:#888888; padding:20px 10px; text-align:right; font-size:6px; font-family:"宋体"; margin-top:20px;
		}
		.pages1 a{
			height:20px; line-height:20px; overflow:hidden; color:#666666; font-size:16px; text-align:center; display:inline-block; padding:0 12px; margin:0 4px; border:1px solid #cccccc; -webkit-border-radius:2px; -moz-border-radius:2px; border-radius:2px;
		}
...
<div class="i_bg bg_color">
	...
	<div class="m_content">
		...
		<div class="m_right">
            ...
            <table border="0" class="order_tab" style="width:930px; text-align:left; margin-bottom:30px;" cellspacing="0" cellpadding="0">                    
              <c:forEach items="${comment }" varStatus="status" var="comment">
	           <tr>
	             <td>	           
		               用户姓名：<font color="#ff4e00">${comment.users.username }</font>&nbsp;
		                商品编号：<font color="#ff4e00">${comment.product.pid }</font>&nbsp; 
		                商品名称：<font color="#ff4e00">${comment.product.pname }</font>&nbsp; 
		                评论编号：<font color="#ff4e00">${comment.commentid }</font>&nbsp;
		                服务星评：<font color="#ff4e00">${comment.commentservice }</font>&nbsp; 
		                物流星评：<font color="#ff4e00">${comment.commentshopping }</font>&nbsp;
		                 商品星评：<font color="#ff4e00">${comment.commentgoods }</font> 		                            
	             </td>
	           </tr>  
                <tr>
                	<td><textarea class="add_txt">${comment.commenttxt }</textarea></font>
                	<img id="commentImg" width="90" height="90" src="commentImg/${comment.commentimg }" /></td>
                </tr>
                <tr>
	                <td>
	                	评论时间:<font color="#ff4e00"><fmt:formatDate value="${comment.commenttime }" pattern="yyyy-MM-dd" /></font>&nbsp;
	                	<input type="button" onclick="btn1(${comment.commentid})" style="color:#fff" value="清空" class="btn_tj">
		    			&nbsp; 							    			
		    			<input type="button" onclick="btn2(${comment.commentid})" style="color:#fff" value="修改" class="btn_tj">
		    			&nbsp; 							    			
		    			<input type="button" onclick="btn3(${comment.commentid})" style="color:#fff" value="删除" class="btn_tj">
	                </td>
                </tr>            
              </c:forEach>
             ...
             <div class="pages1">
                <c:if test="${pageInfo.hasPreviousPage==true }">                  
                   <a href="${pageContext.request.contextPath }/userComment/queryCommentById.action?pageNum=${pageInfo.pageNum-1}&id=${current_user.userid}"><font style="font-size: 1px">上一页</font></a>
                </c:if>
                <c:if test="${pageInfo.hasPreviousPage==false }">
                	<a><font style="font-size: 1px">上一页</font></a>
                </c:if> 
                <c:forEach items="${pageInfo.navigatepageNums }" var="n">
                	<a href="${pageContext.request.contextPath }/userComment/queryCommentById.action?pageNum=${n}&id=${current_user.userid}"><font style="font-size: 1px">${n}</font></a>
                </c:forEach>
                <c:if test="${pageInfo.hasNextPage==true }">                  
                   <a href="${pageContext.request.contextPath }/userComment/queryCommentById.action?pageNum=${pageInfo.pageNum+1}&id=${current_user.userid}"><font style="font-size: 1px">下一页</font></a>
                </c:if>
                <c:if test="${pageInfo.hasNextPage==false }">                  
                   <a><font style="font-size: 1px">下一页</font></a>
                </c:if> 
           </div>
        </div>
    </div>
...
function btn1(id){
	if(!confirm("您确定要清空您的评论记录吗？")){
 		   ...
 		   $.ajax({
 			   url:"${pageContext.request.contextPath }/userComment/cleanComment.action?id="+id,
 			   type:"post",
 			   success:function(result){
 				   	  if(result.code==100){
 				   		alert("清空成功！");
  					   window.location.href="${pageContext.request.contextPath }/userComment/queryCommentById.action?id="+${current_user.userid};
 				   	  }else{
 				   		alert("清空失败！");
   					   window.location.href="${pageContext.request.contextPath }/userComment/queryCommentById.action?id="+${current_user.userid};
 				   	  }					   
...
/*点击修改*/
function btn2(id){
	window.location.href="${pageContext.request.contextPath }/userComment/queryCmtByIdForUpdate.action?id="+id; 
}
...
function btn3(id){
	if(!confirm("您确定要删除您的评论记录吗？")){
 		   ...
 		   $.ajax({
 			   url:"${pageContext.request.contextPath }/userComment/deleteComment.action?id="+id,
 			   type:"delete",
 			   success:function(result){
 				   	  if(result.code==100){
 				   		alert("删除成功！");
