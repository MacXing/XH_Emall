		...
		<div class="m_right">
            ...
            <table border="0" class="order_tab" style="width:930px; text-align:left; margin-bottom:30px;" cellspacing="0" cellpadding="0">          
             <!--  <tr>                                                                                                                                                    
              ...
              <c:forEach items="${msgList }" varStatus="status" var="message">
	           <tr>
	             <td>
		               用户编号：<font color="#ff4e00">${message.users.userid }</font>&nbsp;
		               用户姓名：<font color="#ff4e00">${message.users.username }</font>&nbsp;
		                留言编号：<font color="#ff4e00">${message.msgid }</font>             
	             </td>
	           </tr>  
                <tr>
                	<td><textarea class="add_txt">${message.msgtxt }</textarea></font></td>
                </tr>
                <tr>
	                <td>
	                	留言时间:<font color="#ff4e00"><fmt:formatDate value="${message.msginittime }" pattern="yyyy-MM-dd HH:mm:ss" /></font>&nbsp;
	                	<input type="button" onclick="btn1(${message.msgid})" style="color:#fff" value="清空" class="btn_tj">
		    			&nbsp; 							    			
		    			<input type="button" onclick="btn2(${message.msgid})" style="color:#fff" value="修改" class="btn_tj">
		    			&nbsp; 							    			
		    			<input type="button" onclick="btn3(${message.msgid})" style="color:#fff" value="删除" class="btn_tj">
	                </td>
                </tr>            
              </c:forEach>
            </table>          
            <div class="pages1">
                <c:if test="${pageInfo.hasPreviousPage==true }">                  
                   <a href="${pageContext.request.contextPath }/userCenter/queryUsermessageById.action?pageNum=${pageInfo.pageNum-1}&id=${current_user.userid}"><font style="font-size: 1px">上一页</font></a>
                </c:if>
                <c:if test="${pageInfo.hasPreviousPage==false }">
                	<a><font style="font-size: 1px">上一页</font></a>
                </c:if> 
                <c:forEach items="${pageInfo.navigatepageNums }" var="n">
                	<a href="${pageContext.request.contextPath }/userCenter/queryUsermessageById.action?pageNum=${n}&id=${current_user.userid}"><font style="font-size: 1px">${n}</font></a>
                </c:forEach>
                <c:if test="${pageInfo.hasNextPage==true }">                  
                   <a href="${pageContext.request.contextPath }/userCenter/queryUsermessageById.action?pageNum=${pageInfo.pageNum+1}&id=${current_user.userid}"><font style="font-size: 1px">下一页</font></a>
                </c:if>
...
function btn1(id){
	if(!confirm("您确定要清空您的留言记录吗？")){
 		   ...
 		   $.ajax({
 			   url:"${pageContext.request.contextPath }/userCenter/cleanMessage.action?id="+id,
 			   type:"post",
 			   success:function(result){
 				   	  if(result.code==100){
 				   		alert("清空成功！");
  					   window.location.href="${pageContext.request.contextPath }/userCenter/queryUsermessageById.action?id="+${current_user.userid};
 				   	  }else{
 				   		alert("清空失败！");
   					   window.location.href="${pageContext.request.contextPath }/userCenter/queryUsermessageById.action?id="+${current_user.userid};
 				   	  }					   
...
/*点击修改*/
function btn2(id){
	window.location.href="${pageContext.request.contextPath }/userCenter/queryUsermessageByIdForUpdate.action?id="+id; 
}
...
function btn3(id){
	if(!confirm("您确定要删除您的留言记录吗？")){
 		   ...
 		   $.ajax({
 			   url:"${pageContext.request.contextPath }/userCenter/deleteMessage.action?id="+id,
 			   type:"delete",
 			   success:function(result){
 				   	  if(result.code==100){
 				   		alert("删除成功！");
